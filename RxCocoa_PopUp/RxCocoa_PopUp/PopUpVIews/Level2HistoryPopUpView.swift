...
enum MonthType {
  case One, Three, Six, Direct
  case None
}


class Level2HistoryPopUpView {
    
    private static let sharedInstance = Level2HistoryPopUpView()
    ...
    class func show() {
        ...
        directInputButton.rx.tap
            .map{MonthType.Direct}
            .asDriver(onErrorJustReturn: .None)
            .drive(monthlyStateRelay)
            .disposed(by: sharedInstance.disposeBag)
